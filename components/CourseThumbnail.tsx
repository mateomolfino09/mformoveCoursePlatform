    ...
    <>
        {items?.map((item: Item, index: number) => (
          <div className='w-full h-full flex items-center justify-center flex-col' key={item.id}>
            <div className={`w-[90%] max-h-40 rounded-sm md:rounded px-12 flex justify-center items-center space-x-12 py-8 ${items.indexOf(item) == actualClassIndex && 'bg-[#333333]'}`} key={item.id}>
              ...
              <div className={`absolute top-[64px] left-0 right-0 bottom-0 flex-col space-y-48 justify-between z-[1]`}></div>
                ...
                <Grid item xs={12}>
                  <PrettoSlider 
                    min={0}
                    max={1}
                    value={classes && course ? classes[index].actualTime == 0 ? 0 : parseFloat(((classes[index].actualTime) / course?.classes[index].totalTime).toString()) : 0}
                    />
                </Grid>
                </Grid>
              </div>
