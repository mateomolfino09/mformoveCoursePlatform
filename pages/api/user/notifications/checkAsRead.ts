...
const checkAsRead = async (req: any, res: any) => {
    ...
    try {
      if (req.method === "PUT") {
        ...
        dbUser.notifications.filter((x: Notification) => !x.read).slice(-5).length > 0 ? dbUser.notifications.filter((x: Notification) => !x.read).slice(-5).forEach((noti: Notification) => {
            noti.read = true
