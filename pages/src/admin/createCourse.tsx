...
const CreateCourse = ({ user }: Props) => {
    ...
    }, [moduleNumbers, setModuleNumbers])
    ...
    async function handleSubmit(event: any) {
      ...
      if(name.length < 5) {
        ...
        try {
            ...
            {name, playlistId, imgUrl, password, userEmail, description, price, currencys, moduleNumbers, breakpointTitles , cantidadClases}, 
            config
            )
        ...
        {loading ? (
          ...
          <>
            ...
            <form className='relative mt-24 space-y-4 rounded border-2 border-black/75 bg-black/50 py-12 px-8 my-12 md:max-w-lg md:px-14' autoComplete="nope" onSubmit={handleSubmit}> 
              <h1 className='text-4xl font-semibold mb-12'>Agregar un Curso</h1>
              ...
              <label className='inline-block w-full'>
                ...
                <div className="flex flex-row space-x-2 justify-center items-start">
                  <label className='inline-block w-full'>
                      ...
                      autoComplete="off"
                      onChange={e => {
                        +e.target.value < 0 ? null : setCantidadClases(+e.target.value)
                        setModuleNumbers([1])
                      }}
                      min={0}
                      step={1}
                ...
                <input type="number"
                      ...
                      autoComplete="off"
                      onChange={e => {
                        if(cantidadClases && +e.target.value > cantidadClases) {
                          e.preventDefault()
                        }
                        else {
                          +e.target.value < 0 ? null : setModules(+e.target.value)
                  ...
                  <div className= {`${!showBreakpoints && 'hidden'} text-sm flex space-x-2 items-center`}>
                    <p className={`${!showBreakpoints && 'hidden'} text-sm`}>Selecciona los breakpoints de los módulos</p>
                  ...
                  <div className={`flex space-y-4 flex-row justify-start !mt-0 items-end w-full h-full flex-wrap ${!showBreakpoints && 'hidden'}`}>
                      {classesNumbers.map((number: number) => (
                        <>
                          <div onClick={() => handleModuleSelection(number + 1)} className={`max-w-[2rem] mr-4 h-8  rounded-full justify-center items-center flex cursor-pointer hover:bg-white hover:text-black ${moduleNumbers.includes(number + 1) ? 'bg-white text-black' : 'bg-black'}`} style={{flex: '0 1 21%'}}>
                            <p>{number + 1}</p>
                          </div>   
                        </>
                      ))}
                  </div>
                  <div className={`flex space-y-4 flex-col justify-start !mt-4 items-start w-full h-full flex-wrap ${!showBreakpoints || moduleNumbers.length !== modules && 'hidden'}`}>
                    ...
                    <>
                      ...
                      {moduleNumbers.map((number: number) => (
                        <div className="flex w-full flex-row justify-center items-center">
                          <div onClick={() => handleModuleSelection(number)} className={`min-w-[2rem] mr-4 h-8  rounded-full justify-center items-center flex cursor-pointer hover:bg-white hover:text-black ${moduleNumbers.includes(number) ? 'bg-white text-black' : 'bg-black'}`}>
                            <p>{number}</p>
                          </div>   
                            <label className='inline-block w-full'>
                                <input type="text"
                                placeholder={`Título módulo ${moduleNumbers.indexOf(number) + 1}`} 
                                className='input'
                                onChange={e => handleModuleTitle(e.target.value, moduleNumbers.indexOf(number), number)}
                                />
                            </label>
                      ...
                      {files.length != 0 ? (
                        <>
                          <div className="grid place-items-center input h-80 border-dashed border-2 border-white/80 relative" {...getRootProps()}>
                        ...
                        <div className="grid place-items-center input h-80 border-dashed border-2 border-white/80" {...getRootProps()}>
