...
const MyCourses = ({ courses, user }: Props) => {
  ...
  console.log(courses[0].classes.map((c: ClassesDB) => c.totalTime).reduce((prev, next) => prev + next))
            ...
            <tbody>
              {courses.map((course: CoursesDB) => (
                <tr key={course ? +course._id : 0} ref={ref}>
                  ...
                  <th className="border-solid border border-collapse text-base bg-gray-900/70 opacity-75 px-1">{course.classes.length}</th>
                  <th className="border-solid border border-collapse text-base bg-gray-900/70 opacity-75 px-1">{Math.floor(course.classes.map(c => c.totalTime).reduce((prev, current) => prev + current) / 60 / 60)
                  ...
                  <th className="border-solid border border-collapse text-base bg-gray-900/70 opacity-75 px-1">{course.currency
                  } {course.price}</th>
