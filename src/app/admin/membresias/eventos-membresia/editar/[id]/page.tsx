...
export default function EditEventMembershipPage() {
  ...
  const fetchEvent = async () => {
    ...
    try {
      setFetching(true);
      const res = await fetch(`/api/membership-events/${id}`, { credentials: 'include', cache: 'no-store' });
      ...
      router.push('/admin/membresias/eventos-membresia');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/membership-events/${id}`, {
        method: 'PATCH',
    ...
    <AdmimDashboardLayout>
      <div className="w-full max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          <Link
            href="/admin/membresias/eventos-membresia"
            className="p-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-700"
          >
            <ArrowLeftIcon className="w-5 h-5" />
          </Link>
          <h1 className="text-2xl font-bold text-gray-900 font-montserrat">Editar evento Cuerpo autónomo</h1>
        </div>
