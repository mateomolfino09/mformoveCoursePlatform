  ...
  useEffect(() => {
    async function fetchModule() {
      try {
        ...
        if (!res.ok) {
          router.push('/admin/membresias/modulos-clase');
          return;
        }
  ...
  return (
    <AdmimDashboardLayout>
      <div className="w-full max-w-2xl mx-auto px-4 py-8">
        <Link href="/admin/membresias/modulos-clase" className="inline-flex items-center gap-2 text-gray-900 hover:text-[#4F7CCF] mb-6 font-montserrat">
              ...
              {submodules.map((s) => (
                <div key={s.slug} className="mb-4 last:mb-0 p-4 rounded-lg border border-gray-200 bg-gray-50/80">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-medium text-gray-900 font-montserrat">{s.name}</span>
                    <Link
                      href={`/admin/membresias/submodulos/${id}/${encodeURIComponent(s.slug)}`}
