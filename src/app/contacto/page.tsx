...
const ContactPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    reason: searchParams.get('reason') || 'general',
    message: ''
  });
  ...
  const getReasonLabel = (reason: string) => {
    switch (reason) {
      ...
      case 'feedback':
        return 'Feedback sobre el servicio';
      case 'other':
        return 'Otro motivo';
      default:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/contacto', {
        ...
        body: JSON.stringify({
          ...formData,
          reason: formData.reason?.trim() || 'general',
        }),
      });
            ...
            {submitted ? (
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 font-montserrat">
                  ...
                  {/* Razón (opcional; por defecto consulta general — cumple requisitos nombre + email + mensaje) */}
                  <div>
                    ...
                    {(formData.reason || 'general') && (
                      <p className="mt-2 text-xs text-gray-500 font-montserrat">
                        {getReasonLabel(formData.reason || 'general')}
                      </p>
                    )}
                  </div>
