...
const CreateCourseStepOne = ({ step1ToStep2, step1ToStep0 }: Props) => {
  const dispatch = useDispatch<AppDispatch>();
  const createCourse = useAppSelector(
    (state) => state.createCourseReducer.value
  );
  const {
    classesQuantity: cantidadClasesReg,
    modules: modulesReg,
    modulesNumbers: moduleNumbersReg,
    ...
    showBreakpoints: showBreakpointsReg
  } = createCourse;
  ...
  const [cantidadClases, setCantidadClases] = useState<number | null>(
    cantidadClasesReg
  );
  ...
  const [showBreakpoints, setShowBreakpoints] =
    useState<boolean>(showBreakpointsReg);
    ...
    <div className='relative flex w-full items-start flex-col bg-transparent md:items-center md:justify-center md:bg-transparent'>
      {start && (
        <AdminGuideModal
          ...
          textArr={[
            'Elige la cantidad de clases y la cantidad de módulos que tendrá el curso.',
      ...
      >
        ...
        <div className='w-full flex pt-12 justify-between items-center'>
          <h1 className='text-4xl font-light '>Agregar un Curso</h1>
          <p>Paso 2</p>
        </div>
        <form
          ...
          <div className='space-y-8'>
            ...
            <div className='flex flex-row space-x-2 justify-center items-start'>
              ...
              <div className='absolute right-16 top-28 text-start h-full'>
                <QuestionMarkCircleIcon
                  className='w-6 '
                  onClick={handleVisibleModuleHelper}
                />
              </div>
            </div>
            <div
                    ...
                    >
                      ...
                      <label className='inline-block w-full'>
                        <input
                          ...
                          value={
                            breakpointTitles[moduleNumbers.indexOf(number)]
                          }
                        />
                      </label>
                    </div>
