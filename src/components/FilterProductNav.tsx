...
import { clearData, clearDataFilters, deleteLargo, deleteLevel, deleteOrder, setLargo, setLevel, setOrder, setSeen, toggleNav } from '../redux/features/filterProduct';
...
const FilterProductNav = ({ showNav }: Props) => {  
      ...
      const { filters } = filterProductSlice;
      ...
      const handleFilterChange = (f: ClassTypes, v: ValuesFilters) => {
        ...
        if(f.name.toLowerCase() === "largo") {
          const lengthSelector = filterProductSlice.largo;
          ...
          const levelSelector = filterProductSlice.nivel;
          ...
          const orderSelector = filterProductSlice.ordenar;
                    ...
                    <div key={i} className={`${i === 0 && 'hidden'} w-full px-6 flex flex-col justify-center items-start`} >
                      ...
                      <div className='w-full flex justify-between items-center' onClick={() => {
                        ...
                        <div className='flex space-x-4 items-center'>
                          {filterProductSlice[Object.keys(filterProductSlice)[Object.keys(filterProductSlice).findIndex(x => x === f.name.toLowerCase())]]?.length > 0 && (
                            <>
                              <div className='border border-white md:border-black px-1'>
                                <p className='text-white md:text-black text-sm md:text-base'>{f.values.filter(x => x.value === filterProductSlice[Object.keys(filterProductSlice)[Object.keys(filterProductSlice).findIndex(x => x === f.name.toLowerCase())]][0]
                              )[0].label}</p> 
                              </div>         
                            </>
                          )}
                          {filterProductSlice[Object.keys(filterProductSlice)[Object.keys(filterProductSlice).findIndex(x => x === f.name.toLowerCase())]]?.length > 1 && (
                            <>
                              <div className='border border-white md:border-black px-1'>
                      ...
                      <div className={`${expand === i ? 'max-h-[500px]' : 'max-h-0'} w-full px-2 transition-all duration-500 overflow-hidden`} >
                        <div className='w-full flex flex-wrap justify-start items-center space-x-3 py-2'>
                          {filters[i].values.map(val => (
                            <>
                              <div key={val.id} onClick={() => handleFilterChange(filters[i], val)} className={`rounded-full max-w-[100px] flex justify-center items-center min-h-[15px] px-2 py-1 border mt-2 transition-all duration-200 ${filterProductSlice[Object.keys(filterProductSlice)[Object.keys(filterProductSlice).findIndex(x => x === f.name.toLowerCase())]]?.includes(val.value) ? 'md:border-white md:bg-black bg-white border-black' : 'md:border-black border-white'}`} style={{ flex: '1 0 21%'}}>
                                <p className={`transition-all duration-200 ${filterProductSlice[Object.keys(filterProductSlice)[Object.keys(filterProductSlice).findIndex(x => x === f.name.toLowerCase())]]?.includes(val.value) ? 'md:text-white text-black' : 'md:text-black text-white'}`}>{val.label}</p>
                              </div>
                            </>
                          ))}
