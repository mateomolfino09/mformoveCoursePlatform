...
const Success = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/user/cursos', { credentials: 'include', cache: 'no-store' })
      ...
      .then((data) => {
        ...
        if (match?.invitacionGrupoWhatsapp) {
  ...
  const handlePrimaryCta = () => {
    ...
    if (isCurso && productId) {
      fetch(`/api/user/cursos`, { credentials: 'include', cache: 'no-store' })
        ...
        .catch(() => router.push(routes.navegation.membership.library));
      return;
    }
    router.push(routes.navegation.membership.library);
  };
  ...
  const mainMessage = isCurso
    ? isPreventaCheckout
      ...
      : 'Tu pago se procesó correctamente. En breve tendrás acceso al curso y recibirás un email de confirmación.'
  ...
  const ctaLabel = isCurso ? 'Ir al contenido del curso' : 'Empezar ahora';
      ...
      <section className="relative w-full min-h-[100vh] md:min-h-screen flex items-center justify-center overflow-hidden bg-palette-ink">
        <motion.div className="absolute inset-0">
          <CldImage
            ...
            alt={isCurso ? 'Pago curso confirmado' : 'Cuerpo autónomo Success'}
            fill
            priority
          ...
          {isCurso && invitacionGrupoWhatsapp ? (
            ...
            >
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-6 py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
              >
